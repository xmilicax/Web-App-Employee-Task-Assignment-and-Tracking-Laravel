...
@section('content')
    <form action="{{ route('update_task_admin') }}" method="POST" class="form-group" enctype="multipart/form-data">
        @csrf
        ...
        <div>
            ...
            <!-- očitavanje prethodnih vrednosti ili u slučaju neuspelog kreiranja, uneti podaci se čuvaju u polju  -->
        ...
        <div>
            ...
            <select name="manager" id="manager" required>
                <option value="">Odaberi rukovodioca</option>
                @foreach ($managers as $manager)
                    <option value="{{ $manager->id }}"
                        {{ old('manager', $task['manager']) == $manager->id ? 'selected' : '' }}>
                        {{ $manager->name }}
                    </option>
                @endforeach
            </select>
            ...
            <select name="task_group_id" id="task_group_id" required>
                ...
                @foreach ($task_groups as $task_group)
                    <option value="{{ $task_group->id }}"
                        {{ old('task_group_id', $task['task_group_id']) == $task_group->id ? 'selected' : '' }}>
                        {{ $task_group->name }}
                    </option>
                @endforeach
            </select>
        ...
        <button type="submit" class="button">Izmeni zadatak</button>
    </form>
@endsection
