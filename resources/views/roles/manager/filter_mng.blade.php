...
@section('title')
    <title>Rukovodilac odeljenja - Filter zadataka</title>
@endsection
...
@section('content')
    <div class="form-container">
        <form method="POST" action="{{ route('filterByDeadlineMng') }}">
            @csrf
        ...
        <form method="POST" action="{{ route('filterByExecMng') }}">
            @csrf
        ...
        <form method="POST" action="{{ route('filterByPriorityMng') }}">
            @csrf
            <label for="priority">Prioritet:</label>
            <input type="text" name="priority" placeholder="Unesite prioritet:">
            <button type="submit" class="button3">Filtriraj</button>
        </form>

        <form method="POST" action="{{ route('filterByTitleMng') }}">
            @csrf
    ...
    <a class="left text-pink-700" href="{{route('filter_mng')}}">Poništi filter</a>
